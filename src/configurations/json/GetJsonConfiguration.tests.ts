...
describe("a configuration file with issue link 'prefixes' being a boolean", () => {
	...
	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			"'comet.json' does not declare a valid Comet configuration: 'prefixes' of 'issueLinks' must be an array of strings, but it is a boolean: true",
		)
	})
})
...
describe("a configuration file with issue link 'prefixes' containing invalid items", () => {
	...
	it("raises an error about the first invalid item", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			`'comet.json' does not declare a valid Comet configuration: 'prefixes' of 'issueLinks' must be an array of strings, but it contains an object: { "key": 10 }`,
		)
	})
})
...
describe("a configuration file with issue link 'wildcards' being a string", () => {
	...
	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			`'comet.json' does not declare a valid Comet configuration: 'wildcards' of 'issueLinks' must be an array of strings, but it is a string: *`,
		)
	})
})
...
describe("a configuration file with an unknown option in 'issueLinks'", () => {
	...
	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			"'comet.json' does not declare a valid Comet configuration: 'idunnothisone' is not a valid option of 'issueLinks'",
		)
	})
})
...
describe("a configuration file with 'issueLinks' being null", () => {
	...
	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			"'comet.json' does not declare a valid Comet configuration: 'issueLinks' must be an object, but it is null",
		)
	})
})
...
describe("a configuration file with some rules configured as 'warn'", () => {
	...
	it("raises an error about the first invalid rule", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			`'comet.json' does not declare a valid Comet configuration: 'noBlankSubjectLines' must be 'error', 'off', or an object of 'level' and 'options', but it is a string: warn`,
		)
	})
})
...
describe("a configuration file with some rules configured as numbers", () => {
	...
	it("raises an error about the first invalid rule", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			`'comet.json' does not declare a valid Comet configuration: 'noMergeCommits' must be 'error', 'off', or an object of 'level' and 'options', but it is a number: 0`,
		)
	})
})
...
describe("a configuration file with a rule object that has no level", () => {
	...
	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			"'comet.json' does not declare a valid Comet configuration: 'level' is missing in 'useConciseSubjectLines'",
		)
	})
})
...
describe("a configuration file with a rule object that has a level of 'warn'", () => {
	...
	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			`'comet.json' does not declare a valid Comet configuration: 'level' of 'noRevertRevertCommits' must be 'error' or 'off', but it is a string: warn`,
		)
	})
})
...
describe("a configuration file with a rule object that has a level being a number", () => {
	...
	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			"'comet.json' does not declare a valid Comet configuration: 'level' of 'useCapitalisedSubjectLines' must be 'error' or 'off', but it is a number: 0",
		)
	})
})
...
describe("a configuration file with a rule object that has no options", () => {
	...
	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			"'comet.json' does not declare a valid Comet configuration: 'options' is missing in 'noRestrictedTrailers'",
		)
	})
})
...
`(
	...
	(props: { ruleKey: RuleKey; options: JsonObject; expectedError: string }) => {
		...
		it("raises an error", async () => {
			await expect(getJsonConfiguration()).rejects.toThrow(
				`'comet.json' does not declare a valid Comet configuration: ${props.expectedError}`,
			)
		})
	},
)
...
describe("a configuration file with a rule object containing invalid items", () => {
	...
	it("raises an error about the first invalid item", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			"'comet.json' does not declare a valid Comet configuration: 'patterns' of 'useAuthorEmailPatterns' must be an array of strings, but it contains a number: 42",
		)
	})
})
...
describe("a configuration file with a rule object containing an unknown option", () => {
	...
	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			"'comet.json' does not declare a valid Comet configuration: 'unknownOption' is not a valid option of 'useEmptyLineBeforeBodyLines'",
		)
	})
})
...
describe("a configuration file with an unknown rule", () => {
	...
	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			"'comet.json' does not declare a valid Comet configuration: 'unrecognisedRuleName' is not a valid rule",
		)
	})
})

describe("a configuration file with 'rules' being a number", () => {
	beforeEach(() => {
		mockJsonFile("./comet.json", { rules: 31 })
	})

	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			"'comet.json' does not declare a valid Comet configuration: 'rules' must be an object, but it is a number: 31",
		)
	})
})

describe("a configuration file with 'extends' being a boolean", () => {
	beforeEach(() => {
		mockJsonFile("./comet.json", { extends: true })
	})

	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			"'comet.json' does not declare a valid Comet configuration: 'extends' must be a string, but it is a boolean: true",
		)
	})
})

describe("a configuration file with 'extends' being an array of strings", () => {
	beforeEach(() => {
		mockJsonFile("./comet.json", { extends: ["a.json", "b.json"] })
	})

	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			`'comet.json' does not declare a valid Comet configuration: 'extends' must be a string, but it is an array: [ "a.json", "b.json" ]`,
		)
	})
})

describe("a configuration file with an unknown top-level property", () => {
	beforeEach(() => {
		mockJsonFile("./comet.json", { whatIsThis: true })
	})

	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			"'comet.json' does not declare a valid Comet configuration: 'whatIsThis' is not a valid option",
		)
	})
})

describe("a configuration file with a top-level string", () => {
	beforeEach(() => {
	...
	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			"'comet.json' does not declare a valid Comet configuration: The configuration must be an object, but it is a string: Release the robot butler",
		)
	})
})

describe("a configuration file with a top-level array", () => {
	beforeEach(() => {
		mockJsonFile("./comet.json", [{ rules: {}, tokens: {} }])
	})

	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			`'comet.json' does not declare a valid Comet configuration: The configuration must be an object, but it is an array: [ { "rules": {}, "tokens": {} } ]`,
		)
	})
})

describe("a completely empty configuration file", () => {
	beforeEach(() => {
		mockFile("./comet.json", "")
	})
	...
	it("raises an error", async () => {
		await expect(getJsonConfiguration()).rejects.toThrow(
			"'comet.json' does not declare a valid Comet configuration: The configuration must be an object, but it is empty",
		)
	})
})
